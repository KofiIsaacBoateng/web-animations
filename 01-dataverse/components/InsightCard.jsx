...
const InsightCard = ({ imgUrl, title, subtitle, index }) => (
  ...
  >
    ...
    <motion.div
      ...
      className="flex flex-col gap-2 md:w-[45%] self-start"
    >
      <h3 className="font-bold text-white text-[26px] md:text-[32px]">{title}</h3>
      <p className="text-secondary-white text-[18px] md:text-[25px]">{subtitle}</p>
    </motion.div>

    <motion.div
      variants={fadeIn('left', 'spring', index * 0.25, 1)}
      className="hidden md:flex items-center justify-center border rounded-full self-start p-8"
    >
      <img
        src="/arrow.svg"
